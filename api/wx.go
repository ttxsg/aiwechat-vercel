...
// 构造请求数据
	prompt := fmt.Sprintf(`
		...
		%s
		如果没有指定日期，默认使用今天；如果没有金额，请估算一个合理的数值；支付方式只有 支付宝 或微信 或银行卡 ；标签从以下内容选 生活吃喝加买菜 房贷-银行金 医疗保健 水电物业 出行 家人-互动生活穿衣用品 家用设备 电子设备 电话费 旅游 其他 摩托车 网购 学习课程；开支类型从下面选择：其他 日常开支 固定开支 社交娱乐开支 节假日开支 教育和自我提升开支 医疗保健开支 意外或紧急开支!! 交通开支(出行) 加油 购物；注单位默认是公司单位（例如 ***公司）。；标签从下面选择：固定工资 ，兼职写代码 ，家人资助，人情往来礼金，其他兼职；
		返回的 JSON 格式如下：
		[
			{
				"名称": "买水果",
